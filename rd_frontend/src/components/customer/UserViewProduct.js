...
function UserViewProduct() {
    ...
    useEffect(() => {
        axios.post("http://localhost:4000/api/item/getall")
            .then(res => {
                    ...
                    <> 
                       ...
                       <div className="card" style={{ width: "18rem" }}>
                        ...
                        <div className="card-body">
                            <h5 className="card-title">Item Name: {el.itemName}</h5>

                            <h5 className="card-title">item Description: {el.itemDes}</h5>
                            <p className="card-text">
                      Item Price : {el.itemPrice}
                            </p>
                        <p className="card-text">
                      Item Shipping Charge : {el.itemshippingCharge}
                            </p>
                        <Link to={'/user/booking/'+el._id} class="btn btn-primary">Add to Cart</Link>
                        </div>
                    </div>
                     </div>
